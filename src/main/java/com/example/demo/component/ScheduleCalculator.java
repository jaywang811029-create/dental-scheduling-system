...
public class ScheduleCalculator {
    ...
    public void evenSchedulingLogic(int[] ampmWorkArray, int[] amWorkArray,
        ...
        // logger.warn("刪除前人數{}", assistants.size());
        // removeAssistantLeave(assistants,ShiftEnum.MORNING_AFTERNOON,amDTO.getDate());// 刪除休假員工
        // logger.warn("刪除後人數{}", assistants.size());
    ...
    public int[] nightSchedulingLogic(
        ...
        for (Assistants assistant : assistants) {
            ...
            if (assistant.isFrontDesk() && pmNightArray[0] >= 2) {
                ...
                if (!checkDuplicate(assistant.getName(), nightDTO)
                        && !checkDuplicate(assistant.getName(), pmDTO)
                        && pmDTO.getFloaters().size() < nightWorkArray[2]) //ex: [1,3,3]、晚[1,1,1] 流動被固定晚吃掉一個剩兩個中午，需比對晚上位置有沒有人
                        {
                    pmDTO.getFloaters().add(assistant.getName());//
            ...
            if (assistant.isFrontDesk() && pmNightArray[0] > 0) {
                ...
                if (!checkDuplicate(assistant.getName(), pmDTO) && ampmArray[0] > 0) {
                    ...
                    if (!checkDuplicate(assistant.getName(), pmDTO)
                        ...
                        // 新增總時數
                        assistant.setTotalHours(assistant.getTotalHours() + ShiftEnum.NIGHT.getHours());
                        // logger.warn("員工:{}<日期:{},總時數{}",assistant.getName(),pmDTO.getDate(),assistant.getTotalHours());
                        assistantRepository.save(assistant);
                        continue;
                    }
    ...
    public void test(Assistants assistant,ShiftEnum shiftType,String region){

       boolean isOnDuty =  resultScheduleRepository.checkNameExist(null, region, region);
       if(isOnDuty){
       }
     ...
     * 過濾休假的人員，請誇區人員補上
     */
    public void removeAssistantLeave(List<Assistants> assistants,ShiftEnum shiftType,String date) {

        if(date.equals("2025-08-29")){
            System.out.println("");
        }
        ...
        assistants.removeIf(assistant -> {
            LeaveScheduleKey key = new LeaveScheduleKey(assistant.getId(), localDate);
            return leaveScheduleRepository.findByKey(key).size() > 0;
        });

        if(shiftType.equals(ShiftEnum.FULL) || shiftType.equals(ShiftEnum.MORNING_AFTERNOON)){
            List<Assistants> crossAssistant = assistantFinder.findByDayAndShift(localDate,shiftType.getCode(),45,"%竹%竹%");
            crossAssistant.removeAll(assistants);
            assistants.addAll(crossAssistant);
        }
